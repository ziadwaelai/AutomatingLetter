...
from ..utils.connection_pool import ConnectionPool, PooledConnectionManager
...
class GoogleSheetsService:
    ...
    def __init__(self, pool_size: int = 5):
        """
        Initialize Google Sheets service with connection pooling.
    ...
    def _create_sheets_client(self):
        ...
        try:
            ...
            logger.debug("Google Sheets client created from pool factory")
            return client
    ...
    def get_client(self):
        ...
        IMPORTANT: Must use with context manager or explicitly release.
        """
        try:
    ...
    def client(self):
        ...
        DEPRECATED: Use get_client_context() context manager instead to ensure
        connections are properly released back to the pool.

        For backward compatibility, this property acquires a client but doesn't
        release it automatically. This can lead to connection exhaustion.
        """
    ...
    def get_client_context(self):
        ...
        Usage:
            with service.get_client_context() as client:
                data = client.open("sheet").get_all_values()
                # Client automatically released when exiting context
        """
        return PooledConnectionManager(self._pool)

    def open_sheet_by_id(self, sheet_id: str):
        """
        Helper to open a sheet by ID using pooled connection.
        Use this for external code that needs sheet access.
    ...
    def open_sheet(self, spreadsheet_name: str):
        """
        Helper to open a sheet by name using pooled connection.
        Use this for external code that needs sheet access.
        ...
        with ErrorContext("get_data_by_key", {"key": key, "sheet": sheet_name}):
            try:
                with self.get_client_context() as client:
                    worksheet = client.open(self.config.database.letters_spreadsheet).worksheet(sheet_name)
    ...
    def close_pool(self) -> None:
        ...
        logger.info("Google Sheets connection pool closed")
...
class GoogleDriveService:
    """Enhanced Google Drive service with connection pooling and better error handling."""
    ...
    def __init__(self, pool_size: int = 5):
        ...
        # Create connection pool
        self._pool = ConnectionPool(
            connection_factory=self._create_drive_service,
            cleanup_func=None,  # Google API client doesn't need explicit cleanup
            pool_size=pool_size,
            connection_timeout=30,
            max_connection_lifetime=3600,  # 1 hour
            max_connection_idle_time=300  # 5 minutes
    ...
    def _create_drive_service(self):
        ...
        try:
            ...
            logger.debug("Google Drive service created from pool factory")
    ...
    @property
    def service(self):
        """Get a Google Drive service from the pool."""
        try:
            return self._pool.acquire()
        except Exception as e:
            logger.error(f"Failed to acquire service from pool: {e}")
            raise

    def _release_service(self, service):
        """Release a service back to the pool."""
        self._pool.release(service)

    def _get_service_context(self):
        """
        Get a context manager for safe service acquisition and release.

        Usage:
            with service._get_service_context() as drive_service:
                drive_service.files().create(...).execute()
                # Service automatically released when exiting context
        """
        return PooledConnectionManager(self._pool)
    ...
    def upload_file(
        ...
        with ErrorContext("drive_upload", {"file_path": file_path, "folder_id": folder_id}):
            ...
            try:
                with self._get_service_context() as service:
    ...
    def close_pool(self) -> None:
        """Close all connections in the pool."""
        self._pool.close_all()
        logger.info("Google Drive connection pool closed")
